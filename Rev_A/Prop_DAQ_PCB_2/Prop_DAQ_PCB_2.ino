...
void setup() {
  ...
  txMessage.identifier = 0x21;           // Example identifier
  ...
  while (1) {
    ...
    for (int channel = 0; channel < 4; ++channel) {
      ...
      if (printADS[0] == 1){
        ...
        txMessage.identifier = 0x22;           // Example identifier
        txMessage.flags = TWAI_MSG_FLAG_EXTD;  // Example flags (extended frame)
        txMessage.data_length_code = 8;        // Example data length (8 bytes)
        txMessage.data[2*channel] = value/256;              
        txMessage.data[2*channel+1] = value%256;              
    ...
    for (int channel = 0; channel < 4; ++channel) {
      ...
      txMessage.identifier = 0x23;           // Example identifier
      txMessage.flags = TWAI_MSG_FLAG_EXTD;  // Example flags (extended frame)
      txMessage.data_length_code = 8;        // Example data length (8 bytes)
      txMessage.data[2*channel] = value/256;              
      txMessage.data[2*channel+1] = value%256;              
    ...
    Serial.println(v1);
    Serial.println(v2);
    float tempmillivolt = 1000.0*f_val*(v1-v2);
    ...
    Serial.println("ADS1115 2 differential 0 2 in mV:");
    Serial.println(ads_1115_2.toVoltage()*1000*ads_1115_2.readADC_Differential_0_2());
    for (int channel = 0; channel < 4; ++channel) {
      ...
      txMessage.identifier = 0x24;           // Example identifier
      txMessage.flags = TWAI_MSG_FLAG_EXTD;  // Example flags (extended frame)
      txMessage.data_length_code = 8;        // Example data length (8 bytes)
      txMessage.data[2*channel] = value/256;              
      txMessage.data[2*channel+1] = value%256;              
    ...
    Serial.println(v1);
    Serial.println(v2);
    tempmillivolt = 1000.0*f_val*(v1-v2);
    Serial.print("Voltdiff mV: ");
    Serial.println(tempmillivolt,4);

    kfactor = (16.397-(-5.891))/(400-(-200)); //4.096 mV per 100 C
    temp = tempmillivolt/kfactor;
    Serial.print("Kfactor: ");
    Serial.println(kfactor,4);
